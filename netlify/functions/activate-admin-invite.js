// netlify/functions/activate-admin-invite.js
// Validates a custom admin-invite token stored in Firestore and redirects to a
// freshly-generated Firebase password-reset link (valid for 1 hr from click).
// Mirrors activate-invite.js (the tenant version) but targets /admin.html and
// the adminInviteTokens collection.
//
// GET /api/activate-admin-invite?token={uuid}
//
...
<body>
  <div class="card">
    <div class="header"><span>Admin Portal</span></div>
    <div class="body">
      <h2>${title}</h2>
      <div class="alert">${message}</div>
      ${siteUrl ? `<a href="${siteUrl}/admin.html" class="btn">Back to Admin Login →</a>` : ''}
      <p class="hint">If you continue having issues, ask the Super Admin who invited you to resend the link.</p>
    </div>
  </div>
</body>
</html>`;
...
exports.handler = async (event) => {
  ...
  if (!token) {
    return { statusCode: 400, headers: { 'Content-Type': 'text/html' }, body: errorPage('Invalid Link', 'This activation link is missing required information.', siteUrl) };
  }
  ...
  try {
    ...
    if (snap.empty) {
      return { statusCode: 404, headers: { 'Content-Type': 'text/html' }, body: errorPage('Link Not Found', 'This activation link is invalid or was replaced by a newer one.', siteUrl) };
    }
    ...
    if (tokenData.used) {
      return { statusCode: 410, headers: { 'Content-Type': 'text/html' }, body: errorPage('Link Already Used', 'This activation link has already been used. Ask a Super Admin to resend it, or use "Forgot password?" on the login screen.', siteUrl) };
    }

    const expiresAt = tokenData.expiresAt?.toDate ? tokenData.expiresAt.toDate() : new Date(tokenData.expiresAt);
    if (new Date() > expiresAt) {
      await tokenDoc.ref.update({ used: true, expiredAt: a.firestore.FieldValue.serverTimestamp() });
      return { statusCode: 410, headers: { 'Content-Type': 'text/html' }, body: errorPage('Link Expired', 'This activation link has expired. Ask a Super Admin to resend it.', siteUrl) };
    }
    ...
    return { statusCode: 302, headers: { Location: activationUrl }, body: '' };

  } catch (err) {
    console.error('activate-admin-invite error:', err);
