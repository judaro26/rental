...
exports.handler = async (event) => {
  ...
  try {
    ...
    const { name, category, tenantId, propertyId, propertyWide, adminOnly,
            documentGroupId, chunkIndex, totalChunks, originalName } = fields;
    if (!name) return { statusCode: 400, body: JSON.stringify({ error: 'Document name is required' }) };

    // Chunked-upload fields (present when the client split a large file into parts).
    const isChunked   = !!(documentGroupId && totalChunks && Number(totalChunks) > 1);
    const chunkIdx    = isChunked ? Number(chunkIndex) : null;
    const chunkTotal  = isChunked ? Number(totalChunks) : null;
    ...
    const ref = await a.firestore().collection('documents').add({
      ...
      uploadedBy:  'admin',
      // Chunked-upload metadata — null/absent for normal single-file uploads.
      documentGroupId: isChunked ? documentGroupId : null,
      chunkIndex:      isChunked ? chunkIdx   : null,
      totalChunks:     isChunked ? chunkTotal : null,
      originalName:    isChunked ? (originalName || name) : null,
    });

    return {
      statusCode: 200,
      body: JSON.stringify({
        success: true, docId: ref.id, url: viewUrl, storagePath: blobKey,
        chunkIndex: chunkIdx, totalChunks: chunkTotal,
      }),
    };
  } catch (err) {
