...
exports.handler = async (event) => {
  ...
  const eventRef = db.collection('processedWebhookEvents').doc(stripeEvent.id);
  try {
    ...
    console.warn('Idempotency check failed, proceeding anyway:', e.message);
  }
  ...
  try {
    ...
    // Mark this event as processed so a Stripe retry becomes a no-op.
    try {
      await eventRef.set({
        type: stripeEvent.type,
        processedAt: fb.firestore.FieldValue.serverTimestamp(),
      });
    } catch (e) {
      console.warn('Failed to record processed event id:', e.message);
    }
  } catch (err) {
